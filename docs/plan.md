...
<details>
<summary>v1 架構（已完成，點擊展開）</summary>
...
sequenceDiagram
    ...
    CB->>AL: pub {session_id, messages, response, has_transfer}

    Note over EV: 累積同一 session_id 的訊息

    AL-->>EV: sub（累積同 session）
    Note over EV: session 結束觸發：has_transfer=true 或 idle > 5min
...
**app-log message schema (v2 - from KafkaCallback):**
```json
{
  "session_id": "uuid",
  "timestamp": "ISO8601",
...
**Session 結束觸發：**
1. 訊息帶有 `has_transfer: true`（transfer_to_human tool 被呼叫）→ 立即 evaluate
2. Timeout: session idle > 300s / 5 min（每次收到新訊息時檢查所有 session）
...
## Golden Set (5 組) - v1 參考用

<details>
<summary>v1 golden set（保留供參考，v2 不使用）</summary>

預先準備，存在 `golden_set.json`，v1 供 Gradio「Run All Test Cases」批次展示用。
...
<details>
<summary>v1 phases（全部完成，點擊展開）</summary>
...
- `async_log_success_event` → pub to `app-log`（含 session_id, has_transfer）
...
- 觸發：has_transfer=true 或 idle > 5 min
